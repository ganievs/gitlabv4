...
func run() error {
	...
	schema, err := loadSchema(gitlabToken)
	if err != nil {
		return err
	}
...
func loadSchema(gitlabToken string) (schema interface{}, err error) {
	req, err := http.NewRequest("GET", "https://gitlab.com/api/graphql", nil)
	if err != nil {
		return nil, err
	}
...
func t(text string) *template.Template {
	...
	return template.Must(template.New("").Funcs(template.FuncMap{
		...
		"enumIdentifier": func(enum, value string) string {
			brandNames := map[string]string{ // Augments the list in ident.
				"LINKEDIN": "LinkedIn",
				"YOUTUBE":  "YouTube",
			}
